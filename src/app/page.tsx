...
import { PDATA, SRCLABEL, fmt } from '@/lib/data';
import { useAlerts } from '@/components/alert-provider';
import { Badge, PriDot, Waveform } from '@/components/micro';
import { Ic } from '@/components/icons';

export default function DashboardPage() {
  const { queue, currentId, isPlaying, isPaused, history, ackAlert, delAlert, togglePause, skipCurrent } = useAlerts();
    ...
    <div>
      ...
      <div className={npClass}>
        ...
        <div className="np-ctrls">
          <button className="npbtn npbtn-p" onClick={togglePause} title={isPaused ? 'เล่นต่อ' : 'หยุดชั่วคราว'}>
            {isPaused ? <Ic.play /> : <Ic.pause />}
          </button>
          {current && (
            <button className="npbtn npbtn-s" onClick={() => ackAlert(current.id)} title="รับทราบ — ข้ามรายการนี้">
              <Ic.check />
            </button>
          )}
          {pending.length > 1 && (
            <button className="npbtn npbtn-s" onClick={skipCurrent} title="ข้ามไปรายการถัดไป">
              <Ic.skip />
            </button>
          )}
        ...
        {pending.map((a) => (
          <div key={a.id} className={`qi${a.id === currentId ? ' cur' : ''}`}>
            ...
            <div className="qi-acts">
              <button className="smb ack" onClick={() => ackAlert(a.id)} title="รับทราบ"><Ic.check /></button>
              <button className="smb del" onClick={() => delAlert(a.id)} title="ลบ"><Ic.trash /></button>
            </div>
          </div>
        ))}
      </div>
